...
struct xarecover_st {
  ...
  xarecover_st(XA_recover_txn_list *txnlist,
      const mem_root_unordered_set<my_xid> *clist,
  ...
  {
    assert((commit_list && binlog_xa_prepared_engine &&
    ...
    found_my_xids = 0;
    dry_run = false;
    m_do_binlog_recovery = (commit_list != nullptr);
  }
...
int fetch_xa_prepared()
{
  ...
  plugin_foreach(nullptr, fetch_xa_prepared_handlerton,
...
int ha_recover(const mem_root_unordered_set<my_xid> *commit_list,
  ...
  DBUG_TRACE;
  info.dry_run = (info.do_binlog_recovery() == false &&
  		tc_heuristic_recover == TC_HEURISTIC_NOT_USED);

  assert(g_did_binlog_recovery == 0);
  /* commit_list and tc_heuristic_recover cannot be set both */
  assert(info.do_binlog_recovery() == false ||
         tc_heuristic_recover == TC_HEURISTIC_NOT_USED);
...
bool Sql_cmd_xa_commit::trans_xa_commit(THD *thd) {
  ...
  if (!xid_state->has_same_xid(m_xid)) {
    // Committing a prepared xa txn that was prepared in another connection.
...
bool Sql_cmd_xa_rollback::trans_xa_rollback(THD *thd) {
  ...
  Clone_handler::XA_Operation xa_guard(thd);
  if (!xid_state->has_same_xid(m_xid)) {
    // Aborting a prepared xa txn that was prepared in another connection.
...
bool Sql_cmd_xa_start::trans_xa_start(THD *thd) {
  ...
  if (m_xa_opt != XA_NONE)
    ...
    my_error(ER_XAER_OUTSIDE, MYF(0));
            ...
            m_xid->get_bqual_length() + m_xid->get_gtrid_length()) == nullptr)))
